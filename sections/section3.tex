...
\begin{eg}\label{example of EqSYT}
  equivariant standard tableauxとそうでないものの例を図\ref{eg of EqSYT}で与えた．
  \begin{figure}
    \centering
    \begin{tikzpicture}
      \begin{scope}
...
\begin{prop}\label{main theorem 2}
...
\begin{proof}
  ...
  定理\ref{tableaux calculation}，定理\ref{puzzle calculation}より，
  \begin{align*}
    ...
    &=C^\nu_{\lambda\mu} + \sum_{T\notin\varphi(\mathcal{P}^\nu_{\lambda\mu})}\text{wt}(T)
  \end{align*}
  よって$\sum_{T\notin\varphi(\mathcal{P}^\nu_{\lambda\mu})}\text{wt}(T)=0$である．$\text{wt}(T)$は定義から，$y_2-y_1,\cdots,y_{m+k}-y_{m+k-1}$の多項式で表した時の係数がすべて正である．したがって$\varphi(\mathcal{P}^\nu_{\lambda\mu})=\mathcal{P}^\nu_{\lambda\mu}$でなければならない．すなわち$\varphi$は全射である．
\end{proof}
