...
\begin{defin}
  ...
  \begin{figure}[htbp]\label{puzzle piece}
    \centering
    \begin{tikzpicture}
      \coordinate (A) at (0,0);
      \coordinate (B) at (2,0);
...
\begin{eg}
  ...
  \begin{figure}[htbp]\label{0110,1001,1010}
    \centering
    \begin{tikzpicture}
    ...
    \end{tikzpicture}
    \caption{$\partial P=\Delta^{1010}_{0110,1001}$をみたすただ一つのpuzzle}
  \end{figure}
\end{eg}
...
\begin{figure}[htbp]\label{puzzle weight}
  \centering
  \begin{tikzpicture}
    \draw (0,0)--(8,0)--(4,6.8)--cycle;
  ...
  \end{tikzpicture}
  \caption{equivariant pieceのウェイトの定義．図のequivariant pieceについて$\text{wt}(p)=y_6-y_2$である．}
\end{figure}
...
$\lambda,\mu,\nu\in\binom{n}{k}$とする．puzzle $P\in\mathcal{P}^\nu_{\lambda\mu}$は一般に次の形をしている(figure 1, \cite{Kreiman})．
...
\begin{tikzpicture}[baseline=1pt]
  ...
  \draw[thick] (0,0)-- ++(0.5,0)-- ++(-0.25,0.5)--cycle;
  \node at (0,0.3) {$0$};
  \node at (0.5,0.3) {$0$};
  \node at (0.25,-0.15) {$0$};
\end{tikzpicture}
\begin{tikzpicture}[baseline=2pt]
  ...
  \draw[thick] (0,0.5)-- ++(0.5,0)-- ++(-0.25,-0.5)--cycle;
  \node at (0.25, 0.65) {$0$};
  \node at (0,0.3) {$0$};
  \node at (0.55,0.3) {$0$};
\end{tikzpicture}
のそれぞれのからなり，$a^i_j, b^i_j, c^i_j$は各puzzle pieceの個数を表す．
...
\begin{tikzpicture}[baseline=2pt]
  \fill[black] (0,0.5)-- ++(0.5,0)-- ++(-0.25,-0.5)--cycle;
\end{tikzpicture}
で挟まれた部分をsegmentと呼ぶ(figure2)．またsegmentに含まれる
\begin{tikzpicture}[baseline=-1pt]
  \fill[cyan] (0,0)--(0.25,0.42)--(0.5,0)--(0.25,-0.42)--cycle;
  \draw[thick] (0,0)--(0.25,0.42)--(0.5,0)--(0.25,-0.42)--cycle;
\end{tikzpicture}
...
\begin{tikzpicture}[baseline=2pt]
  ...
  \draw[thick](0,0)-- ++(0.5,0)-- ++(0.25,0.42)-- ++(-0.5,0)--cycle;
\end{tikzpicture}
のpuzzle pieceの個数をそのsegmentの長さと呼ぶ

\input{./sections/figure2.tex}

puzzle $P$の各path $p$に対して次の操作を行って得られるedge labeled tableauxを$\varphi(P)$とする．

$p$のsegmentを上から順に$s_1,\cdots,s_a$, $(a\leq k)$とする．$s_1$に
\begin{tikzpicture}[baseline=-1pt]
  \fill[cyan] (0,0)--(0.25,0.42)--(0.5,0)--(0.25,-0.42)--cycle;
  \draw[thick] (0,0)--(0.25,0.42)--(0.5,0)--(0.25,-0.42)--cycle;
\end{tikzpicture}
...
\begin{center}
  ...
  $a_1=2,b_1=3,a_2=2,c=2$
\end{center}

$\Lambda=\overbrace{(n,\cdots,n)}^{k\text{-copies}}$の$k$行目に次のようにedge labelとbox labelを書き入れていく．まず左から$a_1$個のboxのedge labelに$1$から$a_1$を1つずつ，左から単調増大になるように入れる．次の右隣りの$b_1$個のboxには何もせず，次の$a_2$個のboxのedge labelに$a_1+1$から$a_1+a_2$を単調増大になるように入れていく．これを$b_u$まで行ったのち，$c$個のboxに$a_1+\cdots+a_u+1$から$a_1+\cdots+a_u+c$までを左から単調増大になるように入れる．

\begin{center}
  \begin{tikzpicture}
    \begin{scope}
    \fill[cyan] (0,0) rectangle (1.6, 0.8);
    \fill[lightgray] (1.6,0) rectangle (4,0.8);
    \fill[cyan] (4,0) rectangle (5.6,0.8);
    \fill[yellow] (5.6,0) rectangle (7.2,0.8);
    \draw (0,0) rectangle (7.2, 0.8);
    \foreach \x in {1,...,8} {
      \draw (\x * 0.8, 0) -- ++(0, 0.8);
    }
...
次のsegment $s_2$に対して，$\Lambda$の$k-1$行目のedge labelとbox labelを同様の方法で書き入れていく．ただし$k$行目で最後に処理を行ったboxのすぐ右上のboxから処理を始める．また$s_1$の長さが$0$であった場合は，$k-1$行目の最も左にある箱から処理を行う．同様にしてすべてのsegmentに対してedge labelとbox labelを書き入れていく．$s_i$の長さが$0$であったなら，$k-i+1$行の処理はスキップして，$s_{i+1}$に対する処理は，直前に処理を行ったboxのすぐ右の列にある$k-i$行目のboxから始める．
...
\begin{figure}[htbp]
  \caption{
  のpathに対する処理．
  $s_3$の長さが$0$であるから，図のように$3$行目に対する処理をスキップする．}
\end{figure}

この処理をすべてのpathに対して行って得られるedge labeled tableauxを$\varphi(P)$とする．

\begin{eg}
  図\ref{eg of varphi}のpuzzle $P$に対して$\varphi(P)$を計算する．
  \begin{figure}[htbp]\label{eg of varphi}
    \centering
    \begin{tikzpicture}
      ...
      \foreach \x in {1,...,7}{
        \draw[gray] (\x, 0) -- (\x/2, \x);
        \draw[gray] (\x, 0) -- (4+\x/2, 8-\x);
        \draw[gray] (\x/2,\x) -- (8-\x/2,\x);
      }
      
      \fill[yellow] (0,0)-- ++(1,0)-- ++(1/2,1)--++(-1,0)--cycle;
      \draw (0,0)-- ++(1,0)-- ++(1/2,1)--++(-1,0)--cycle;
      \fill[cyan] (3/2,1)-- ++(1/2,1)-- ++(-1/2,1)-- ++(-1/2,-1);
      \draw  (3/2,1)-- ++(1/2,1)-- ++(-1/2,1)-- ++(-1/2,-1);
    \end{tikzpicture}
    \caption{}
  \end{figure}
\end{eg}
